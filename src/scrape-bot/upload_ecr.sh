...
docker images | grep -e "$TAG_BOT_ID.*$TAG_BOT_VERSION"

# Step 3:
# Tag your image with the Amazon ECR registry, repository, and optional image tag name combination to use
echo "Tag Docker Image"
IMAGE_ID=$(docker images --filter=reference="$TAG_BOT_ID:$TAG_BOT_VERSION" --format "{{.ID}}")
docker tag "$IMAGE_ID" "$AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$TAG_BOT_ID:$TAG_BOT_VERSION"

# Step 4:
# Push image to a docker repository
echo "Push Docker Image"
docker push "$AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$TAG_BOT_ID:$TAG_BOT_VERSION"
